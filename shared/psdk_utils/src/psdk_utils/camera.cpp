...
CVector2D camera::crosshair_offset() {
  if (weapon_in_hand() == eWeaponType::WEAPON_SNIPERRIFLE) return {0.0f, 0.0f};
  ...
  auto fx = math::pi() / 2.0f -
            std::atan2(1.0f,
                       mult * 2.0f * (camera.m_f3rdPersonCHairMultX - 0.5f)),
       fz = math::pi() / 2.0f -
            std::atan2(1.0f,
                       mult * (2.0f * (0.5f - camera.m_f3rdPersonCHairMultY) *
...
bool camera::is_widescreen_fix_on() {
  return RsGlobal.ps->fullScreen &&
         plugin::patch::GetUInt(0x53D7B1 + 0x1) != 0x1C1C6A;
}
